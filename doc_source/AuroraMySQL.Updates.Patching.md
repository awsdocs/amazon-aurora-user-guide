...
+ [Upgrading Aurora MySQL by applying pending maintenance to an Aurora MySQL DB cluster](#AuroraMySQL.Updates.PendingMaintenance) \(prior to Aurora MySQL 1\.19\.0 or 2\.03\.2\)

 For information about how zero\-downtime patching can reduce interruptions during the upgrade process, see [Zero\-downtime patching](#AuroraMySQL.Updates.ZDP)\. 
...
+  **By using the Amazon RDS console** – Modify the properties of your cluster\. In the **Modify DB cluster** window, change the Aurora MySQL engine version in the **DB engine version** box\. If you aren't familiar with the general procedure for modifying a cluster, follow the instructions at [Modifying the DB cluster by using the console, CLI, and API](Aurora.Modifying.md#Aurora.Modifying.Cluster)\. 
...
+  **By using the Amazon RDS console** – Complete the following steps: 
  ...
  1.  Locate the DB cluster where you intend to turn automatic minor version upgrade on or off\. 

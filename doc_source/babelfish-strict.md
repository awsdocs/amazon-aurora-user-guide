...
You can use the `sp_babelfish_configure` stored procedure to control the settings of an escape hatch\. Use the script to set the escape hatch to `ignored` or `strict`\. If it's set to `strict`, Babelfish returns an error that you need to correct before continuing\.
...
+ To list the named escape hatches and their values, for the current session or cluster\-wide, run the command `sp_babelfish_configure hatch_name` where `hatch_name` is the identifier of one or more escape hatches\.
...
| escape\_hatch\_fk\_constraint\_dbname |  When creating a FOREIGN KEY constraint with ALTER TABLE, you can optionally specify the database name of the referenced table, which has no real function\. No escape hatch is needed, but make sure to check that the database name, if specified, is valid\.  |  strict  | 
...
| escape\_hatch\_compatibility\_level |  Controls Babelfish behavior related to the COMPATIBILITY\_LEVEL clause in ALTER DATABASE\.  |  ignore  | 
...
| escape\_hatch\_select\_for\_xml\_explicit |  Controls Babelfish behavior related to the XML EXPLICIT clause\. When set to strict, the server raises an error \(`EXPLICIT mode isn't supported`\) at the SELECT\.\.\.FROM XML EXPLICIT clause\. When this value is included in a stored procedure, the procedure is created without error but raises an error when run\.  |  strict  | 
| escape\_hatch\_select\_for\_xml\_auto |  When this value is set to strict, the server raises an error \(`EXPLICIT mode isn't supported`\) at the SELECT\.\.\.FROM XML AUTO clause\. When this value is included in a stored procedure, the procedure is created without error but raises an error when run\.  |  strict  | 
...
| escape\_hatch\_constraint\_asc\_desc |  When declaring a PRIMARY KEY or UNIQUE constraint, in SQL Server you can specify an optional ASC or DESC keyword, which PostgreSQL doesn't support for constraints\. Babelfish accepts the ASC and DESC keywords for CREATE INDEX, which Aurora PostgreSQL also supports\)\. When the DESC keyword is ignored, the index is created but with ascending sorting\.   |    | 
...
| quoted\_identifier\_off |  Controls Babelfish behavior toward strings in double quotation marks and SET QUOTED\_IDENTIFIER OFF\. This escape hatch suppresses the error message returned when Babelfish encounters strings in double quotation marks and helps resolve issues for clients like sqmcmd\.  |  ignore  | 

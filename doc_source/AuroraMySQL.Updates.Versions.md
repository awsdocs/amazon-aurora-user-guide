...
**Note**  
 ...
 For Aurora MySQL 2\.x, all versions 2\.03\.1 and lower are represented by the engine version `5.7.12`\. In the same way, all versions prior to 1\.19\.0 are represented by the engine version `5.6.10a`\. These older version designations don't include the `5.7.mysql_aurora` prefix\. When you specified `5.7.12` or `5.6.10a` while creating or modifying a cluster, you got the highest version prior to the 2\.03\.2 and 1\.19\.0 versions where the version numbering changed\. To determine the exact version number for those older versions, you used the SQL technique explained in [Checking Aurora MySQL versions using SQL](#AuroraMySQL.Updates.DBVersions)\. 
...
**Tip**  
 ...
 Prior to Aurora MySQL 1\.19\.0 and 2\.03\.2, the process to update the engine version is to **Apply a Pending Maintenance Action** in the cluster\. This process does not change the Aurora MySQL engine version displayed by the AWS Management Console\. Thus, if you see an Aurora MySQL cluster with a reported engine version of `5.6.10a` or `5.7.12`, to find out the specific version you connect to the cluster and query the `AURORA_VERSION` system variable as described previously\. 
...
```
 ...
 The version numbers returned by the AWS Management Console, AWS CLI, and RDS API using the techniques described in [Checking or specifying Aurora MySQL engine versions through AWS](#AuroraMySQL.Updates.EngineVersions) are typically more descriptive\. However, for versions prior to 2\.03\.2 and 1\.19, AWS always returns the version numbers `5.7.12` or `5.6.10a`\. For those older versions, you use the SQL technique to check the precise version number\. 

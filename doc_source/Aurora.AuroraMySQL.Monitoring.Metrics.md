...
|  `AuroraGlobalDBProgressLag`  |  In an Aurora Global Database, the measure of how far the mirror cluster is behind the primary cluster for both user transactions and system transactions\. It is calculated by subtracting the mirror MCL time stamp from the primary VCL \(volume complete LSN\) time stamp\.  |  Aurora MySQL and Aurora PostgreSQL  |  Milliseconds  | 
...
|  `AuroraGlobalDBRPOLag`  |  In an Aurora Global Database, the RPO lag time\. This metric measures how far the mirror cluster is behind the primary cluster for user transactions\. It is calculated by subtracting the last user commit time stamp in the mirror from the last user commit time stamp in the primary\.  |  Aurora MySQL and Aurora PostgreSQL  |  Milliseconds  | 

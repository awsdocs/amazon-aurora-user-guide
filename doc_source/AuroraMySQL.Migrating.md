...
| An RDS for MySQL DB snapshot | Physical |  You can migrate data directly from an RDS for MySQL DB snapshot to an Amazon Aurora MySQL DB cluster\. For details, see [Migrating data from a MySQL DB instance to an Amazon Aurora MySQL DB cluster by using a DB snapshot](AuroraMySQL.Migrating.RDSMySQL.md)\.  | 

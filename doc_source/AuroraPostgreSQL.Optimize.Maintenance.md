...
1. Set the status of the plan to `preferred` so that the optimizer will choose to run it instead of select from the set of approved plans when the minimum\-cost plan is not already `approved` or `preferred`\.
...
1. Turn off manual plan capture and enforce the use of managed plans\.
   ...
   Now, when the original SQL statement runs, the optimizer will choose either an `approved` or `preferred` plan\. If the minimum\-cost plan is neither `approved` nor `preferred`, then the optimizer will choose the `preferred` plan\.
...
For example, use the `apg_plan_mgmt.delete_plan` function as follows to delete all plans that haven't been chosen as the minimum\-cost plan or haven't run in at least 31 days\. However, this example doesn't delete plans that have been explicitly rejected\.
...
**Note**  
If you are dumping from one version of the `apg_plan_mgmt` extension and restoring into a different version of the `apg_plan_mgmt` extension, then the columns in the plans table might be different\. In that case, name the columns explicitly instead of using SELECT \*\. 
